...
export function generateStaticParams() {
  const mcps = getEntitiesByType('mcp');
...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  if (!entity) {
    return {
      title: 'MCP Not Found | Guard0 TrustVector',
    };
  }

  const score = calculateOverallScore(entity);
  const title = `${entity.name} MCP Security Evaluation | Guard0 TrustVector`;
  const description = `Guard0 TrustVector: ${entity.name} by ${entity.provider} - Model Context Protocol security assessment with ${score}/100 trust score. MCP security evaluation for secure AI agent deployments.`;
  ...
  return {
    ...
    openGraph: {
      ...
      url: `https://trustvector.guard0.ai/mcps/${entity.id}`,
    },
    twitter: {
      card: 'summary_large_image',
      title: `${entity.name} MCP Security | Guard0`,
      description: `MCP security evaluation: ${entity.name} scored ${score}/100. Model Context Protocol security by Guard0.`,
      creator: '@Guard0Security',
    },
    alternates: {
      canonical: `/mcps/${entity.id}`,
    },
  };
}
      ...
      <div className="border-b border-border">
        <div className="container mx-auto px-4 py-4">
          <nav className="flex items-center gap-2 text-sm text-muted-foreground uppercase tracking-wide">
            ...
            <Link href="/?type=mcp" className="hover:text-foreground transition-colors font-bold">MCPs</Link>
            <ChevronRight className="w-4 h-4" />
          ...
          <div className="max-w-5xl mx-auto">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {entity.tags?.slice(0, 4).map((tag) => (
                    <span key={tag} className="inline-flex items-center px-3 py-1 bg-muted border border-border rounded text-xs font-bold uppercase">{tag}</span>
                  ))}
                </div>
              </div>
            ...
            <div className="bg-primary/5 border-l-2 border-primary rounded-r-lg p-6 mb-6">
              <div className="flex items-center gap-2 text-primary font-bold text-sm uppercase tracking-wide mb-2">
                <Star className="w-4 h-4" />About This MCP
              </div>
            ...
            <div className="flex flex-wrap items-center gap-6 text-sm">
              ...
              {entity.website && (
                <a href={entity.website} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-primary hover:underline font-bold uppercase">
                  ...
                  <div className="space-y-2 text-sm">
                    {Object.entries(entity.metadata).map(([key, value]) => {
                      if (typeof value === 'object' && value !== null) {
                        return (
                          <div key={key}>
                            <div className="font-bold uppercase text-xs mb-1">{key.replace(/_/g, ' ')}</div>
